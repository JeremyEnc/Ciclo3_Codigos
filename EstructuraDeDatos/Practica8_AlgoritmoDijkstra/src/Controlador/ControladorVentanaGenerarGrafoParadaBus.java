...
public class ControladorVentanaGenerarGrafoParadaBus implements Initializable {
    ...
    private void agregarArista(ActionEvent event)
    {
        ...
        if(txtfPeso.getText().isBlank())
        {
            Alert alert = new Alert(Alert.AlertType.ERROR);
            alert.setHeaderText(null);
            alert.setTitle("Error");
            alert.setContentText("Debe añadir un peso");
            alert.showAndWait();
            return;
        }
        ...
        if(Objects.equals(vtI, vtF))
        {
            Alert alert = new Alert(Alert.AlertType.ERROR);
            alert.setHeaderText(null);
            alert.setTitle("Error");
            alert.setContentText("No se puede añadir una arista con el mismo vertice");
            alert.showAndWait();

        }
        else
        {
            ctrlParadaBus.getGrafo().insertarAristaConPeso(vtI, vtF, peso);
            aristas.get(vtI)[vtF+1] = peso.toString();
            tvGrafo.refresh();
            
            cbxVerticeInicial.setValue(null);
            cbxVerticeFinal.setValue(null);
            txtfPeso.setText(null);
        }
    }
    ...
    private void cargarCbxVertices()
    {
        ...
        for (int i = 0; i < ctrlParadaBus.getGrafo().numVertices(); i++)
        {
           ...
           cbxOrigen.getItems().add(ctrlParadaBus.getGrafo().obtenerEtiqueta(i).toString());
           cbxDestino.getItems().add(ctrlParadaBus.getGrafo().obtenerEtiqueta(i).toString());
            
        }
    }

    @FXML
    private void generarCamino(ActionEvent event)
    {
        try 
        {
            Integer origen = cbxOrigen.getSelectionModel().getSelectedIndex();
            Integer destino = cbxDestino.getSelectionModel().getSelectedIndex();
            
            Integer camino[] = ctrlParadaBus.getGrafo().algoritmoDijkstra(origen, destino);
            String caminoString = "";
            ...
            StringBuilder strb = new StringBuilder(caminoString);
            txtArecorrido.setText(strb.reverse().toString());
        } 
        catch (Exception e)
        {
            Alert alert = new Alert(Alert.AlertType.ERROR);
            alert.setHeaderText(null);
            alert.setTitle("Error");
            alert.setContentText("No existe camino posible entre estos vertices");
            alert.showAndWait();
        }
    }
